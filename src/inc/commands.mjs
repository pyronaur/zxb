import utility from "./info.mjs";
import { confirm } from "./helpers.mjs";

async function relink() {
  console.log("Re-creating script links");

  const bin_directory = `${utility.bindDir}`;
  await fs.ensureDir(bin_directory);

  for (const script of await utility.list()) {
    const { bin, file } = utility.paths(script);
...
async function cleanup() {
  ...
  const bins = await utility.bins();
  const utilities = await utility.list();
...
async function create({ slug, file, bin }) {
  ...
  if (commandExists.exitCode !== 1) {
    ...
    console.log(`Command "${slug}" is already aliased to "${alias.trim()}"\n`);
    process.exit();
  }
  $.verbose = v;
...
async function remove({ slug }) {
  ...
  const { file, bin } = utility.paths(slug);
  await $`rm ${file}`;
  await $`rm ${bin}`;
}
